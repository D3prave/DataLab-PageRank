...
async def remote_ram_background_updater():
    ...
    while True:
        oracle_ram_cache, oracle_ram_time = await get_remote_ram("IP", "KEY"), time.time()
        oracle2_ram_cache, oracle2_ram_time = await get_remote_ram("IP", "KEY"), time.time()
        aws_ram_cache, aws_ram_time = await get_remote_ram("IP", "KEY"), time.time()
